...
class GridFlagComponent extends React.Component {

    constructor(props) {
        super(props)
        this.state = {
            text: 'Veuillez choisir votre langue de départ',
            initL: false
        }
    ...
    componentDidMount(){
        if(this.props.initL != undefined) {
            this.setState({text: 'Veuillez choisir votre langue de départ'})
            this.setState({initL: true})
        }
        else {
            this.setState({text: 'Veuillez choisir votre langue de d\'arrivée'})
        }
    }
    ...
    navigation(item) {
        if(this.state.initL) {
            ...
            NavigationActions.initailLanguage({languageVoice: item.key,initialLangage: item.label})
        }
        else {
            this.props.changeTL(item.key.split("-")[0],item.coef,item.key.split("-")[1])
            NavigationActions.FinalLanguage({outputVoice: item.key,finalLangage: item.label})
        }
    }
...
export default connect(mapStateToProps,mapDispatchToProps)(GridFlagComponent);